...
#define LED_BUILTIN 13
  ...
  void system_dynamics(OSQPFloat xn[], OSQPFloat x[], OSQPFloat u[], const OSQPFloat A[], const OSQPFloat B[])
  {
    matrix_vector_reset_mult(NSTATES, NSTATES, A, x, xn);
    matrix_vector_mult(NSTATES, NINPUTS, B, u, xn);
  }

  void compute_q(OSQPFloat q[], const OSQPFloat R[], OSQPFloat uref[])
  // uref is Nh-1 x Nu
    ...
    for (int step = 0; step < NRUNS; step++)
    {
      ...
      { // enable safety filter
        ...
        if (exitflag != 0)
        {
          ...
          Serial.println("OSQP failed to solve the problem!\n");
        }
