...
%tiny_color = '#f79807';
%osqp_color = '#09809B';
tiny_box_color = [.5,0,0];
osqp_box_color = [0,0,.5];
line_type = '--';
box_lw = 1.5;
plot_lw = 1.5;
box_w = .5;
...
avg_data = mean(tinympc_s_result);
min_data = min(tinympc_s_result);
max_data = max(tinympc_s_result);

% Calculate the interquartile range
iqr_data = iqr(tinympc_s_result);

% Determine the upper and lower whisker length
whisker_length_factor = 1.5; % Adjust as needed
lower_whisker_lengths = max(min_data - whisker_length_factor * iqr_data, min(min_data));
upper_whisker_lengths = min(max_data + whisker_length_factor * iqr_data, max(max_data));
% Plot mpc results

% Create a box plot with separate boxes for each column
%setBoxParams(boxplot(tinympc_s_result,xs_tinympc,'Whisker',0), tiny_color, box_lw, tiny_box_color);
